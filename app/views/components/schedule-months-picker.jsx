...
const months = [
    ...
    { id: 9,  label: 'planning.months.september.short' },
 ...
 */
const ScheduleMonthsPicker = ({onClickMonth, selectedMonths}) => (
    <div data-cavimac="schedule-months-picker">
        {
            months.map((month) => {
                const isSelected = indexOf(selectedMonths, month.id) >= 0;
                return <button key={`month-${month.id}`} onClick={(evt) => onClickMonth(evt.target.value)} type='button' value={month.id} data-selected={isSelected}>{translate(month.label)}</button>
            })
        }
    </div>
);
